...
func TestBind(t *testing.T) {
	testdata := []struct {
		Name string
		V    interface{}
		Fn   func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime)
	}{
		{"Fields", bridgeTestFieldsType{"a", "b", "c", "d", "e"}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			t.Run("Exported", func(t *testing.T) {
				v, err := rt.RunString(ctx, `obj.exported`)
				if assert.NoError(t, err) {
					assert.Equal(t, "a", v.Export())
				}
			})
			t.Run("ExportedTag", func(t *testing.T) {
				v, err := rt.RunString(ctx, `obj.renamed`)
				if assert.NoError(t, err) {
					assert.Equal(t, "b", v.Export())
				}
			})
			t.Run("unexported", func(t *testing.T) {
				v, err := rt.RunString(ctx, `obj.unexported`)
				if assert.NoError(t, err) {
					assert.Equal(t, nil, v.Export())
				}
			})
			t.Run("unexportedTag", func(t *testing.T) {
				v, err := rt.RunString(ctx, `obj.unexportedTag`)
				if assert.NoError(t, err) {
					assert.Equal(t, nil, v.Export())
				}
			})
		}},
		{"Methods", bridgeTestMethodsType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			t.Run("unexportedFn", func(t *testing.T) {
				_, err := rt.RunString(ctx, `obj.unexportedFn()`)
				assert.EqualError(t, err, "TypeError: Object has no member 'unexportedFn' at <eval>:1:17(3)")
			})
			t.Run("ExportedFn", func(t *testing.T) {
				_, err := rt.RunString(ctx, `obj.exportedFn()`)
				assert.NoError(t, err)
			})
			t.Run("unexportedPtrFn", func(t *testing.T) {
				_, err := rt.RunString(ctx, `obj.unexportedPtrFn()`)
			...
			t.Run("ExportedPtrFn", func(t *testing.T) {
				_, err := rt.RunString(ctx, `obj.exportedPtrFn()`)
				switch obj.(type) {
		...
		{"Error", bridgeTestErrorType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			_, err := rt.RunString(ctx, `obj.error()`)
			assert.Contains(t, err.Error(), "GoError: error")
		}},
		{"JSValue", bridgeTestJSValueType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			v, err := rt.RunString(ctx, `obj.func(1234)`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(1234), v.Export())
			}
		}},
		{"JSValueError", bridgeTestJSValueErrorType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			_, err := rt.RunString(ctx, `obj.func()`)
		...
		{"JSValueContext", bridgeTestJSValueContextType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			// _, err := rt.RunString(ctx, `obj.func()`)
		...
		{"JSValueContextError", bridgeTestJSValueContextErrorType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			_, err := rt.RunString(ctx, `obj.func()`)
			assert.Contains(t, err.Error(), "GoError: missing argument")
		...
		{"NativeFunction", bridgeTestNativeFunctionType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			v, err := rt.RunString(ctx, `obj.func(1234)`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(1234), v.Export())
			}
		}},
		{"NativeFunctionError", bridgeTestNativeFunctionErrorType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			_, err := rt.RunString(ctx, `obj.func()`)
			assert.Contains(t, err.Error(), "GoError: missing argument")
		...
		{"NativeFunctionContext", bridgeTestNativeFunctionContextType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			// _, err := rt.RunString(ctx, `obj.func()`)
			// assert.Contains(t, err.Error(), "GoError: func() can only be called from within default()")
		...
		{"NativeFunctionContextError", bridgeTestNativeFunctionContextErrorType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			_, err := rt.RunString(ctx, `obj.func()`)
			assert.Contains(t, err.Error(), "GoError: missing argument at")
		...
		{"Add", bridgeTestAddType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			v, err := rt.RunString(ctx, `obj.add(1, 2)`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(3), v.Export())
			}
		}},
		{"AddWithError", bridgeTestAddWithErrorType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			v, err := rt.RunString(ctx, `obj.addWithError(1, 2)`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(3), v.Export())
			}

			t.Run("Negative", func(t *testing.T) {
				_, err := rt.RunString(ctx, `obj.addWithError(0, -1)`)
		...
		{"Context", bridgeTestContextType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			// _, err := rt.RunString(ctx, `obj.context()`)
		...
		{"ContextAdd", bridgeTestContextAddType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			// _, err := rt.RunString(ctx, `obj.contextAdd(1, 2)`)
			// assert.Contains(t, err.Error(), "GoError: contextAdd() can only be called from within default()")

			t.Run("Valid", func(t *testing.T) {
				v, err := rt.RunString(ctx, `obj.contextAdd(1, 2)`)
				if assert.NoError(t, err) {
					assert.Equal(t, int64(3), v.Export())
				}
			})
		}},
		{"ContextAddWithError", bridgeTestContextAddWithErrorType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			// _, err := rt.RunString(ctx, `obj.contextAddWithError(1, 2)`)
			// assert.Contains(t, err.Error(), "GoError: contextAddWithError() can only be called from within default()")

			t.Run("Valid", func(t *testing.T) {
				v, err := rt.RunString(ctx, `obj.contextAddWithError(1, 2)`)
				if assert.NoError(t, err) {
					assert.Equal(t, int64(3), v.Export())
				}

				t.Run("Negative", func(t *testing.T) {
					_, err := rt.RunString(ctx, `obj.contextAddWithError(0, -1)`)
					assert.Contains(t, err.Error(), "GoError: answer is negative")
				})
			})
		}},
		...
		{"Count", bridgeTestCounterType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			...
			case *bridgeTestCounterType:
				for i := 0; i < 10; i++ {
					t.Run(strconv.Itoa(i), func(t *testing.T) {
						v, err := rt.RunString(ctx, `obj.count()`)
						if assert.NoError(t, err) {
			...
			case bridgeTestCounterType:
				_, err := rt.RunString(ctx, `obj.count()`)
				assert.EqualError(t, err, "TypeError: Object has no member 'count' at <eval>:1:10(3)")
			default:
				assert.Fail(t, "UNKNOWN TYPE")
			}
		}},
		{"Sum", bridgeTestSumType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			sum := 0
			args := []string{}
			for i := 0; i < 10; i++ {
		...
		{"SumWithContext", bridgeTestSumWithContextType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			// _, err := rt.RunString(ctx, `obj.sumWithContext(1, 2)`)
		...
		{"SumWithError", bridgeTestSumWithErrorType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			sum := 0
			args := []string{}
			for i := 0; i < 10; i++ {
				args = append(args, strconv.Itoa(i))
		...
		{"SumWithContextAndError", bridgeTestSumWithContextAndErrorType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			// _, err := rt.RunString(ctx, `obj.sumWithContextAndError(1, 2)`)
		...
		{"Constructor", bridgeTestConstructorType{}, func(t *testing.T, ctx context.Context, obj interface{}, rt *Runtime) {
			v, err := rt.RunString(ctx, `new obj.Constructor()`)
			assert.NoError(t, err)
		...
		t.Run(name, func(t *testing.T) {
			for _, data := range testdata {
				t.Run(data.Name, func(t *testing.T) {
					...
					obj := vfn(data.V)
					rt.Bind("obj", obj)
					data.Fn(t, context.Background(), obj, rt)
				})
			}
		})
	}
...
func BenchmarkProxy(b *testing.B) {
	types := []struct {
		Name, FnName string
		Value        interface{}
		Fn           func(b *testing.B, ctx context.Context, fn interface{})
	}{
		{"Fields", "", bridgeTestFieldsType{}, nil},
		{"Methods", "exportedFn", bridgeTestMethodsType{}, func(b *testing.B, ctx context.Context, fn interface{}) {
			f := fn.(func())
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				f()
		...
		{"Add", "add", bridgeTestAddType{}, func(b *testing.B, ctx context.Context, fn interface{}) {
		...
		{"AddError", "addWithError", bridgeTestAddWithErrorType{}, func(b *testing.B, ctx context.Context, fn interface{}) {
			b.Skip()
			f := fn.(func(int, int) int)
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
		...
		{"Context", "context", bridgeTestContextType{}, func(b *testing.B, ctx context.Context, fn interface{}) {
			b.Skip()
			f := fn.(func())
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
		...
		{"ContextAdd", "contextAdd", bridgeTestContextAddType{}, func(b *testing.B, ctx context.Context, fn interface{}) {
			b.Skip()
			f := fn.(func(int, int) int)
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
		...
		{"ContextAddError", "contextAddWithError", bridgeTestContextAddWithErrorType{}, func(b *testing.B, ctx context.Context, fn interface{}) {
			b.Skip()
			f := fn.(func(int, int) int)
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
		...
		{"Sum", "sum", bridgeTestSumType{}, func(b *testing.B, ctx context.Context, fn interface{}) {
			f := fn.(func(...int) int)
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				f(1, 2, 3)
		...
		{"SumContext", "sumWithContext", bridgeTestSumWithContextType{}, func(b *testing.B, ctx context.Context, fn interface{}) {
			b.Skip()
			f := fn.(func(...int) int)
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
		...
		{"SumError", "sumWithError", bridgeTestSumWithErrorType{}, func(b *testing.B, ctx context.Context, fn interface{}) {
			b.Skip()
			f := fn.(func(...int) int)
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
		...
		{"SumContextError", "sumWithContextAndError", bridgeTestSumWithContextAndErrorType{}, func(b *testing.B, ctx context.Context, fn interface{}) {
			b.Skip()
			f := fn.(func(...int) int)
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
		...
		{"Constructor", "Constructor", bridgeTestConstructorType{}, func(b *testing.B, ctx context.Context, fn interface{}) {
