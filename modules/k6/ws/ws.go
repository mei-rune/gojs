...
var ErrWSInInitContext = gojs.NewInitContextError("using websockets in the init context is not supported")
	...
	if handlers, ok := s.eventHandlers[event]; ok {
		for _, handler := range handlers {
			if _, err := handler(goja.Undefined(), args...); err != nil {
				gojs.Throw(gojs.GetRuntime(s.ctx), err)
			}
		}
	}
}
