...
func TestParseHTML(t *testing.T) {
	...
	t.Run("Find", func(t *testing.T) {
		v, err := rt.RunString(ctx, `doc.find("h1")`)
	...
	t.Run("Add", func(t *testing.T) {
		t.Run("Selector", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("h1").add("footer")`)
			if assert.NoError(t, err) && assert.IsType(t, Selection{}, v.Export()) {
				sel := v.Export().(Selection).sel
		...
		t.Run("Selection", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("h1").add(doc.find("footer"))`)
			if assert.NoError(t, err) && assert.IsType(t, Selection{}, v.Export()) {
				sel := v.Export().(Selection).sel
				assert.Equal(t, 2, sel.Length())
				assert.Equal(t, "Lorem ipsumThis is the footer.", sel.Text())
	...
	t.Run("Text", func(t *testing.T) {
		v, err := rt.RunString(ctx, `doc.find("h1").text()`)
		if assert.NoError(t, err) {
			assert.Equal(t, "Lorem ipsum", v.Export())
		}
	})
	t.Run("Attr", func(t *testing.T) {
		v, err := rt.RunString(ctx, `doc.find("h1").attr("id")`)
		if assert.NoError(t, err) {
			assert.Equal(t, "top", v.Export())
		}
		t.Run("Default", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("h1").attr("id", "default")`)
			if assert.NoError(t, err) {
				assert.Equal(t, "top", v.Export())
			}
		})
		t.Run("Unset", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("h1").attr("class")`)
			if assert.NoError(t, err) {
				assert.True(t, goja.IsUndefined(v), "v is not undefined: %v", v)
			}

			t.Run("Default", func(t *testing.T) {
				v, err := rt.RunString(ctx, `doc.find("h1").attr("class", "default")`)
				if assert.NoError(t, err) {
	...
	t.Run("Html", func(t *testing.T) {
		v, err := rt.RunString(ctx, `doc.find("h1").html()`)
		if assert.NoError(t, err) {
			assert.Equal(t, "Lorem ipsum", v.Export())
		}
	})
	t.Run("Val", func(t *testing.T) {
		...
		t.Run("TextArea", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("#textarea").val()`)
			if assert.NoError(t, err) {
	...
	t.Run("Children", func(t *testing.T) {
		t.Run("All", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("head").children()`)
			if assert.NoError(t, err) {
				sel := v.Export().(Selection).sel
				assert.Equal(t, 1, sel.Length())
		...
		t.Run("With selector", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("body").children("p")`)
			if assert.NoError(t, err) {
				sel := v.Export().(Selection).sel
				assert.Equal(t, 2, sel.Length())
	...
	t.Run("Closest", func(t *testing.T) {
		v, err := rt.RunString(ctx, `doc.find("textarea").closest("form").attr("id")`)
		if assert.NoError(t, err) {
			assert.Equal(t, "form1", v.Export())
		}
	})
	t.Run("Contents", func(t *testing.T) {
		v, err := rt.RunString(ctx, `doc.find("head").contents()`)
		if assert.NoError(t, err) {
			sel := v.Export().(Selection).sel
	...
	t.Run("Each", func(t *testing.T) {
		t.Run("Func arg", func(t *testing.T) {
			v, err := rt.RunString(ctx, `{ var elems = []; doc.find("#select_multi option").each(function(idx, elem) { elems[idx] = elem.innerHTML(); }); elems }`)
			var elems []string
		...
		t.Run("Invalid arg", func(t *testing.T) {
			_, err := rt.RunString(ctx, `doc.find("#select_multi option").each("");`)
	...
	t.Run("Is", func(t *testing.T) {
		t.Run("String selector", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("h1").is("h1")`)
			if assert.NoError(t, err) {
				assert.Equal(t, true, v.Export())
			}
		})
		t.Run("Function selector", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("h1").is(function(idx, val){ return val.text() == "Lorem ipsum" })`)
			if assert.NoError(t, err) {
				assert.Equal(t, true, v.Export())
			}
		})
		t.Run("Selection selector", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("body").children().first().is(doc.find("h1"))`)
			if assert.NoError(t, err) {
				assert.Equal(t, true, v.Export())
			}
		})
	...
	t.Run("Filter", func(t *testing.T) {
		t.Run("String", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("body").children().filter("p")`)
			if assert.NoError(t, err) {
				sel := v.Export().(Selection).sel
				assert.Equal(t, 2, sel.Length())
			}
		...
		t.Run("Function", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("body").children().filter(function(idx, val){ return val.is("p") })`)
			if assert.NoError(t, err) {
				sel := v.Export().(Selection).sel
				assert.Equal(t, 2, sel.Length())
			}
		...
		t.Run("Selection", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("body").children().filter(doc.find("p"))`)
			if assert.NoError(t, err) {
				sel := v.Export().(Selection).sel
				assert.Equal(t, 2, sel.Length())
			}
	...
	t.Run("End", func(t *testing.T) {
		v, err := rt.RunString(ctx, `doc.find("body").children().filter("p").end()`)
		if assert.NoError(t, err) {
			sel := v.Export().(Selection).sel
	...
	t.Run("Eq", func(t *testing.T) {
		v, err := rt.RunString(ctx, `doc.find("body").children().eq(3).attr("id")`)
		if assert.NoError(t, err) {
			assert.Equal(t, "form1", v.Export())
		}
	})
	t.Run("First", func(t *testing.T) {
		v, err := rt.RunString(ctx, `doc.find("body").children().first().attr("id")`)
		if assert.NoError(t, err) {
			assert.Equal(t, "top", v.Export())
		}
	})
	t.Run("Last", func(t *testing.T) {
		v, err := rt.RunString(ctx, `doc.find("body").children().last().text()`)
		if assert.NoError(t, err) {
	...
	t.Run("Has", func(t *testing.T) {
		t.Run("String selector", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("body").children().has("input").size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(1), v.Export())
			}
		})
		t.Run("Selection selector", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("body").children().has(doc.find("input")).size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(1), v.Export())
			}
		})
	...
	t.Run("Map", func(t *testing.T) {
		t.Run("Valid", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("#select_multi option").map(function(idx, val) { return val.text() })`)
			if assert.NoError(t, err) {
		...
		t.Run("Invalid arg", func(t *testing.T) {
			_, err := rt.RunString(ctx, `doc.find("#select_multi option").map("");`)
			if assert.Error(t, err) {
				assert.IsType(t, &goja.Exception{}, err)
				assert.Contains(t, err.Error(), "must be a function")
			}
		...
		t.Run("Map with attr must return string", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("#select_multi").map(function(idx, val) { return val.attr("name") })`)
			if assert.NoError(t, err) {
				mapped := v.Export().([]string)
	...
	t.Run("Next", func(t *testing.T) {
		t.Run("No arg", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("h1").next()`)
			if assert.NoError(t, err) {
				sel := v.Export().(Selection).sel
				assert.Equal(t, 1, sel.Length())
		...
		t.Run("Filter arg", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("p").next("form")`)
			if assert.NoError(t, err) {
				sel := v.Export().(Selection).sel
				assert.Equal(t, 1, sel.Length())
			}
	...
	t.Run("NextAll", func(t *testing.T) {
		t.Run("No arg", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("h1").nextAll()`)
			if assert.NoError(t, err) {
				sel := v.Export().(Selection).sel
		...
		t.Run("Filter arg", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("h1").nextAll("p")`)
			if assert.NoError(t, err) {
				sel := v.Export().(Selection).sel
				assert.Equal(t, 2, sel.Length())
			}
	...
	t.Run("Prev", func(t *testing.T) {
		t.Run("No arg", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("footer").prev()`)
			if assert.NoError(t, err) {
				sel := v.Export().(Selection).sel
		...
		t.Run("Filter arg", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("footer").prev("form")`)
			if assert.NoError(t, err) {
				sel := v.Export().(Selection).sel
				assert.Equal(t, 1, sel.Length())
			}
	...
	t.Run("PrevAll", func(t *testing.T) {
		t.Run("No arg", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("form").prevAll()`)
			if assert.NoError(t, err) {
				sel := v.Export().(Selection).sel
				assert.Equal(t, 3, sel.Length())
			}
		...
		t.Run("Filter arg", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("form").prevAll("p")`)
			if assert.NoError(t, err) {
				sel := v.Export().(Selection).sel
				assert.Equal(t, 2, sel.Length())
			}
	...
	t.Run("PrevUntil", func(t *testing.T) {
		t.Run("String", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("footer").prevUntil("h1").size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(3), v.Export())
			}
		})
		t.Run("Query", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("footer").prevUntil(doc.find("h1")).size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(3), v.Export())
			}
		})
		t.Run("String filtered", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("form").prevUntil("h1", "p").size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(2), v.Export())
			}
		})
		t.Run("Query filtered", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("form").prevUntil(doc.find("h1"), "p").size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(2), v.Export())
			}
		})
		t.Run("All", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("footer").prevUntil().size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(4), v.Export())
			}
		})
		t.Run("All filtered", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("footer").prevUntil(null, "p").size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(2), v.Export())
			}
		})
	...
	t.Run("NextUntil", func(t *testing.T) {
		t.Run("String", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("h1").nextUntil("footer").size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(3), v.Export())
			}
		})
		t.Run("Query", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("h1").nextUntil(doc.find("footer")).size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(3), v.Export())
			}
		})
		t.Run("String filtered", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("h1").nextUntil("footer", "p").size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(2), v.Export())
			}
		})
		t.Run("Query filtered", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("h1").nextUntil(doc.find("footer"), "p").size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(2), v.Export())
			}
		})
		t.Run("All", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("h1").nextUntil().size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(4), v.Export())
			}
		})
		t.Run("All filtered", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("h1").nextUntil(null, "p").size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(2), v.Export())
			}
		})
	...
	t.Run("Parent", func(t *testing.T) {
		t.Run("No filter", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("textarea").parent().attr("id")`)
			if assert.NoError(t, err) {
				assert.Equal(t, "form1", v.Export())
			}
		})
		t.Run("Filtered", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("textarea").parent("form").attr("id")`)
			if assert.NoError(t, err) {
				assert.Equal(t, "form1", v.Export())
			}
		})
	...
	t.Run("Parents", func(t *testing.T) {
		t.Run("No filter", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("textarea").parents().size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(3), v.Export())
			}
		})
		t.Run("Filtered", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("textarea").parents("body").size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(1), v.Export())
			}
		})
	...
	t.Run("ParentsUntil", func(t *testing.T) {
		t.Run("String", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("textarea").parentsUntil("html").size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(2), v.Export())
			}
		})
		t.Run("Query", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("textarea").parentsUntil(doc.find("html")).size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(2), v.Export())
			}
		})
		t.Run("String filtered", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("textarea").parentsUntil("html", "body").size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(1), v.Export())
			}
		})
		t.Run("Query filtered", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("textarea").parentsUntil(doc.find("html"), "body").size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(1), v.Export())
			}
		})
		t.Run("All", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("textarea").parentsUntil().size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(3), v.Export())
			}
		})
		t.Run("All filtered", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("textarea").parentsUntil(null, "body").size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(1), v.Export())
			}
		})
	...
	t.Run("Not", func(t *testing.T) {
		t.Run("String selector", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("body").children().not("p").size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(3), v.Export())
			}
		})
		t.Run("Selection selector", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("body").children().not(doc.find("p")).size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(3), v.Export())
			}
		})
		t.Run("Function selector", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("body").children().not(function(idx, val){ return val.is("p") }).size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(3), v.Export())
			}
		})
	...
	t.Run("Siblings", func(t *testing.T) {
		t.Run("No filter", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("form").siblings().size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(4), v.Export())
			}
		})
		t.Run("Filtered", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("form").siblings("p").size()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(2), v.Export())
			}
		})
	...
	t.Run("Slice", func(t *testing.T) {
		t.Run("No filter", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("body").children().slice(1, 2)`)
			if assert.NoError(t, err) {
				sel := v.Export().(Selection).sel
				assert.Equal(t, 1, sel.Length())
				assert.True(t, sel.Is("p"))
		...
		t.Run("Filtered", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("body").children().slice(3)`)
			if assert.NoError(t, err) {
				sel := v.Export().(Selection).sel
				assert.Equal(t, 2, sel.Length())
	...
	t.Run("Get", func(t *testing.T) {
		t.Run("No args", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("body").children().get()`)
			if assert.NoError(t, err) {
		...
		t.Run("+ve index", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("body").children().get(1)`)
			if assert.NoError(t, err) {
	...
	t.Run("ToArray", func(t *testing.T) {
		v, err := rt.RunString(ctx, `doc.find("p").toArray()`)
		if assert.NoError(t, err) {
	...
	t.Run("Index", func(t *testing.T) {
		t.Run("No args", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("p").index()`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(1), v.Export())
			}
		})
		t.Run("String selector", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("form").index("body > *")`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(3), v.Export())
			}
		})
		t.Run("Selection selector", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("body").children().index(doc.find("footer"))`)
			if assert.NoError(t, err) {
				assert.Equal(t, int64(4), v.Export())
			}
		})
	...
	t.Run("Data <p>", func(t *testing.T) {
		t.Run("boolean attr", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("p").data("test-b")`)
			if assert.NoError(t, err) {
				assert.Equal(t, true, v.Export())
			}
		})
		t.Run("snakeCase attr name", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("p").data("testB")`)
			if assert.NoError(t, err) {
				assert.Equal(t, true, v.Export())
			}
		})
		t.Run("empty string", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("p").data("test-empty")`)
			if assert.NoError(t, err) {
				assert.Equal(t, nil, v.Export())
			}
		})
		t.Run("json attr", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("p").data("opts").id`)
			if assert.NoError(t, err) {
