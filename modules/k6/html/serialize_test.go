...
func TestSerialize(t *testing.T) {
	...
	t.Run("SerializeArray", func(t *testing.T) {
		t.Run("form", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("form").serializeArray()`)
		...
		t.Run("controls", func(t *testing.T) {
			v, err := rt.RunString(ctx, `doc.find("input").serializeArray()`)
			if assert.NoError(t, err) {
				arr := v.Export().([]FormValue)
	...
	t.Run("SerializeObject", func(t *testing.T) {
		v, err := rt.RunString(ctx, `doc.find("form").serializeObject()`)
		if assert.NoError(t, err) {
	...
	t.Run("Serialize", func(t *testing.T) {
		v, err := rt.RunString(ctx, `doc.find("form").serialize()`)
		if assert.NoError(t, err) {
			url := v.String()
