...
func (HTML) ParseHTML(ctx context.Context, src string) (Selection, error) {
	...
	return Selection{rt: gojs.GetRuntime(ctx), sel: doc.Selection}, nil
}
...
func (s Selection) Each(v goja.Value) Selection {
	gojaFn, isFn := goja.AssertFunction(v)
	if !isFn {
		gojs.Throw(s.rt, errors.New("Argument to each() must be a function."))
	}
	...
	fn := func(idx int, sel *goquery.Selection) {
		if _, err := gojaFn(v, s.rt.ToValue(idx), selToElement(Selection{s.rt, s.sel.Eq(idx), s.URL})); err != nil {
			gojs.Throw(s.rt, errors.Wrap(err, "Function passed to each() failed."))
		}
	}
...
func (s Selection) Filter(v goja.Value) Selection {
	...
	gojaFn, isFn := goja.AssertFunction(v)
	if !isFn {
		gojs.Throw(s.rt, errors.New("Argument to filter() must be a function, a selector or a selection"))
	}
...
func (s Selection) Is(v goja.Value) bool {
	...
	default:
		gojaFn, isFn := goja.AssertFunction(v)
		if !isFn {
			gojs.Throw(s.rt, errors.New("Argument to is() must be a function, a selector or a selection"))
		}
...
func (s Selection) Map(v goja.Value) []string {
	gojaFn, isFn := goja.AssertFunction(v)
	if !isFn {
		gojs.Throw(s.rt, errors.New("Argument to map() must be a function"))
	}
