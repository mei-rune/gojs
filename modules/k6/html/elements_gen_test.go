...
func TestGenElements(t *testing.T) {
	...
	t.Run("Test text properties", func(t *testing.T) {
		for _, test := range textTests {
			v, err := rt.RunString(ctx, `doc.find("#`+test.id+`").get(0).`+test.property+`()`)
			if err != nil {
	...
	t.Run("Test bool properties", func(t *testing.T) {
		for _, test := range boolTests {
			vT, errT := rt.RunString(ctx, `doc.find("#`+test.idTrue+`").get(0).`+test.property+`()`)
			if errT != nil {
			...
			vF, errF := rt.RunString(ctx, `doc.find("#`+test.idFalse+`").get(0).`+test.property+`()`)
			if errF != nil {
	...
	t.Run("Test url properties", func(t *testing.T) {
		...
		for _, test := range urlTests {
			...
			v, err := rt.RunString(ctx, `urldoc.find("#`+test.id+`").get(0).`+test.property+`()`)
			if err != nil {
