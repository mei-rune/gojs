...
func (s sharedArray) wrap(ctx context.Context, rt *gojs.Runtime) goja.Value {
	cal, err := rt.RunString(ctx, arrayWrapperCode)
	if err != nil {
		gojs.Throw(rt, err)
	}
	call, _ := goja.AssertFunction(cal)
	wrapped, err := call(goja.Undefined(), rt.ToValue(rt.ToBindObject(s)))
	if err != nil {
		gojs.Throw(rt, err)
	}
