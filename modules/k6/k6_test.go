...
func TestFail(t *testing.T) {
	...
	_, err := rt.RunString(ctx, `k6.fail("blah")`)
...
func TestSleep(t *testing.T) {
	...
	for name, d := range testdata {
		t.Run(name, func(t *testing.T) {
			startTime := time.Now()
			_, err := rt.RunString(ctx, `k6.sleep(1)`)
			endTime := time.Now()
	...
	t.Run("Cancel", func(t *testing.T) {
		...
		go func() {
			startTime := time.Now()
			_, err := rt.RunString(ctx, `k6.sleep(10)`)
			endTime := time.Now()
			assert.NoError(t, err)
...
// 		})
// 		_, err = rt.RunString(ctx, `k6.group("my group", fn)`)
// 		assert.NoError(t, err)
// 		assert.Equal(t, state.Group, root)
// 	})

// 	t.Run("Invalid", func(t *testing.T) {
// 		_, err := rt.RunString(ctx, `k6.group("::", function() { throw new Error("nooo") })`)
...
// 	baseCtx := gojs.WithRuntime(context.Background(), rt)
...
// 		_, err := rt.RunString(ctx, `k6.check(null, { "check": true })`)
// 		assert.NoError(t, err)
...
// 			_, err := rt.RunString(ctx, `k6.check(null, { "a": true, "b": false })`)
// 			assert.NoError(t, err)
...
// 		t.Run("Invalid", func(t *testing.T) {
// 			_, err := rt.RunString(ctx, `k6.check(null, { "::": true })`)
// 			assert.Contains(t, err.Error(), "GoError: group and check names may not contain '::'")
// 		})
// 	})
...
// 		_, err := rt.RunString(ctx, `k6.check(null, 12345)`)
// 		assert.NoError(t, err)
...
// 						v, err := rt.RunString(ctx, fmt.Sprintf(tpl, value))
...
// 			v, err := rt.RunString(ctx, `k6.check(null, { "check": true })`)
// 			if assert.NoError(t, err) {
...
// 		v, err := rt.RunString(ctx, `k6.check(null, {"check": true}, {a: 1, b: "2"})`)
// 		if assert.NoError(t, err) {
// 			assert.Equal(t, true, v.Export())
// 		}
