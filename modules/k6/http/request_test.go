...
func newRuntime(
	...
	return tb, state, samples, rt, tb.Context
}
...
func TestRequestAndBatch(t *testing.T) {
	...
	t.Run("Redirects", func(t *testing.T) {
		...
		t.Run("10", func(t *testing.T) {
			_, err := rt.RunString(ctx, sr(`http.get("HTTPBIN_URL/redirect/10")`))
			assert.NoError(t, err)
		})
		t.Run("11", func(t *testing.T) {
			...
			t.Run("Unset Max", func(t *testing.T) {
				...
				exists, logEntry := logtest.LastEntry(observedLogs)
				if assert.True(t, exists) {
					assert.Equal(t, log.WarnLevel, logEntry.Level)
					assert.Equal(t, sr("HTTPBIN_URL/redirect/11"), logEntry.ContextMap()["url"])
					assert.Equal(t, "Stopped after redirects and returned the redirection; pass { redirects: n } in request params or set global maxRedirects to silence this", logEntry.Message)
				}
			})
		})
	...
	t.Run("Timeout", func(t *testing.T) {
		...
		t.Run("10s", func(t *testing.T) {
			...
			exists, _ := logtest.LastEntry(observedLogs)
	...
	t.Run("TLS", func(t *testing.T) {
		t.Run("cert_expired", func(t *testing.T) {
			_, err := rt.RunString(ctx, `http.get("https://expired.badssl.com/");`)
			require.Error(t, err)
	...
	t.Run("Invalid", func(t *testing.T) {
		...
		_, err := rt.RunString(ctx, `http.request("", "");`)
		require.Error(t, err)
		...
		t.Run("throw=false", func(t *testing.T) {
			...
			if assert.True(t, exists) {
				assert.Equal(t, log.WarnLevel, logEntry.Level)
				assert.Contains(t, logEntry.ContextMap()["error"].(error).Error(), "unsupported protocol scheme")
				assert.Equal(t, "Request Failed", logEntry.Message)
			}
		})
	})
	t.Run("Unroutable", func(t *testing.T) {
		_, err := rt.RunString(ctx, `http.request("GET", "http://sdafsgdhfjg/");`)
		assert.Error(t, err)
	})
	...
	t.Run("Params", func(t *testing.T) {
		for _, literal := range []string{`undefined`, `null`} {
			t.Run(literal, func(t *testing.T) {
				_, err := rt.RunString(ctx, fmt.Sprintf(sr(`
	...
	t.Run("Batch", func(t *testing.T) {
		t.Run("error", func(t *testing.T) {
			_, err := rt.RunString(ctx, `var res = http.batch("https://somevalidurl.com");`)
			require.Error(t, err)
		})
		t.Run("GET", func(t *testing.T) {
			_, err := rt.RunString(ctx, sr(`
...
func TestRequestArrayBufferBody(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(tc.arr, func(t *testing.T) {
			_, err := rt.RunString(ctx, sr(fmt.Sprintf(`
...
func TestRequestCompression(t *testing.T) {
	...
	t.Run("custom set header", func(t *testing.T) {
		...
		t.Run("encoding", func(t *testing.T) {
			...
			`))
			require.NoError(t, err)
			require.NotEmpty(t, observedLogs.TakeAll())
		})
...
func TestResponseTypes(t *testing.T) {
	...
	_, err := rt.RunString(ctx, replace(`
...
func BenchmarkHandlingOfResponseBodies(b *testing.B) {
	...
	testResponseType := func(responseType string) func(b *testing.B) {
		...
		return func(b *testing.B) {
			for i := 0; i < b.N; i++ {
				_, err := rt.RunString(ctx, testCode)
				if err != nil {
					b.Error(err)
				}
			}
