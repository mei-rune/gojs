...
var ErrHTTPForbiddenInInitContext = gojs.NewInitContextError("Making http requests in the init context is not supported")

// ErrBatchForbiddenInInitContext is used when batch was made in the init context
var ErrBatchForbiddenInInitContext = gojs.NewInitContextError("Using batch in the init context is not supported")
...
func (h *HTTP) Batch(ctx context.Context, reqsV goja.Value) (goja.Value, error) {
	...
	return gojs.GetRuntime(ctx).ToValue(results), err
}
