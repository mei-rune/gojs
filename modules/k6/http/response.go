...
func (res *Response) JSON(selector ...string) goja.Value {
	...
	if err != nil {
		gojs.Throw(gojs.GetRuntime(res.GetCtx()), err)
	}
	if v == nil {
		return goja.Undefined()
	}
	return gojs.GetRuntime(res.GetCtx()).ToValue(v)
}
...
func (res *Response) HTML(selector ...string) html.Selection {
	...
	case string:
		body = b
	default:
		gojs.Throw(gojs.GetRuntime(res.GetCtx()), errors.New("invalid response type"))
	}
...
func (res *Response) SubmitForm(args ...goja.Value) (*Response, error) {
	...
	if form.Size() == 0 {
		gojs.Throw(rt, fmt.Errorf("no form found for selector '%s' in response '%s'", formSelector, res.URL))
	}
...
func (res *Response) ClickLink(args ...goja.Value) (*Response, error) {
	...
	if link.Size() == 0 {
		gojs.Throw(rt, fmt.Errorf("no element found for selector '%s' in response '%s'", selector, res.URL))
	}
	hrefAttr := link.Attr("href")
	if hrefAttr == goja.Undefined() {
		gojs.Throw(rt, fmt.Errorf("no valid href attribute value found on element '%s' in response '%s'", selector, res.URL))
	}
