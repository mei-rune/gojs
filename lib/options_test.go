...
func TestOptions(t *testing.T) {
	...
	t.Run("TLSCipherSuites", func(t *testing.T) {
		for suiteName, suiteID := range netext.SupportedTLSCipherSuites {
			t.Run(suiteName, func(t *testing.T) {
				opts := Options{}.Apply(Options{TLSCipherSuites: &netext.TLSCipherSuites{suiteID}})
		...
		t.Run("JSON", func(t *testing.T) {
			t.Run("String", func(t *testing.T) {
				...
				assert.Equal(t, &netext.TLSCipherSuites{tls.TLS_ECDHE_RSA_WITH_RC4_128_SHA}, opts.TLSCipherSuites)

				t.Run("Roundtrip", func(t *testing.T) {
					data, err := json.Marshal(opts.TLSCipherSuites)
					assert.NoError(t, err)
					assert.Equal(t, `["TLS_ECDHE_RSA_WITH_RC4_128_SHA"]`, string(data))
					var vers2 netext.TLSCipherSuites
	...
	t.Run("TLSVersion", func(t *testing.T) {
		versions := netext.TLSVersions{Min: tls.VersionSSL30, Max: tls.VersionTLS12}
		opts := Options{}.Apply(Options{TLSVersion: &versions})

		assert.NotNil(t, opts.TLSVersion)
		assert.Equal(t, opts.TLSVersion.Min, netext.TLSVersion(tls.VersionSSL30))
		assert.Equal(t, opts.TLSVersion.Max, netext.TLSVersion(tls.VersionTLS12))
		...
		t.Run("JSON", func(t *testing.T) {
			t.Run("Object", func(t *testing.T) {
				...
				assert.NoError(t, json.Unmarshal([]byte(jsonStr), &opts))
				assert.Equal(t, &netext.TLSVersions{
					Min: netext.TLSVersion(tls.VersionSSL30),
					Max: netext.TLSVersion(tls.VersionTLS12),
				}, opts.TLSVersion)
			...
			t.Run("String", func(t *testing.T) {
				...
				assert.NoError(t, json.Unmarshal([]byte(jsonStr), &opts))
				assert.Equal(t, &netext.TLSVersions{
					Min: netext.TLSVersion(tls.VersionTLS12),
					Max: netext.TLSVersion(tls.VersionTLS12),
				}, opts.TLSVersion)
			})
			t.Run("Blank", func(t *testing.T) {
				...
				jsonStr := `{"tlsVersion":""}`
				assert.NoError(t, json.Unmarshal([]byte(jsonStr), &opts))
				assert.Equal(t, &netext.TLSVersions{}, opts.TLSVersion)
			})
			t.Run("Not a string", func(t *testing.T) {
				var opts Options
	...
	t.Run("TLSAuth", func(t *testing.T) {
		tlsAuth := []*netext.TLSAuth{
			{TLSAuthFields: netext.TLSAuthFields{
	...
	t.Run("Hosts", func(t *testing.T) {
		host, err := netext.NewHostAddress(net.ParseIP("192.0.2.1"), "80")
		assert.NoError(t, err)

		opts := Options{}.Apply(Options{Hosts: map[string]*netext.HostAddress{
...
func TestCIDRUnmarshal(t *testing.T) {
	testData := []struct {
		input          string
		expectedOutput *netext.IPNet
		expectFailure  bool
	}{
		{
			"10.0.0.0/8",
			&netext.IPNet{IPNet: net.IPNet{
				IP:   net.IP{10, 0, 0, 0},
...
func TestHostAddressUnmarshal(t *testing.T) {
	testData := []struct {
		input          string
		expectedOutput *netext.HostAddress
		expectFailure  string
	}{
		{
			"1.2.3.4",
			&netext.HostAddress{IP: net.ParseIP("1.2.3.4")},
			"",
		},
		{
			"1.2.3.4:80",
			&netext.HostAddress{IP: net.ParseIP("1.2.3.4"), Port: 80},
			"",
		},
		{
			"1.2.3.4:asdf",
		...
		},
		{
			...
			&netext.HostAddress{IP: net.ParseIP("2001:0db8:0000:0000:0000:ff00:0042:8329")},
			"",
		},
		{
			"2001:db8::68",
			&netext.HostAddress{IP: net.ParseIP("2001:db8::68")},
			"",
		},
		{
			"[2001:db8::68]:80",
			&netext.HostAddress{IP: net.ParseIP("2001:db8::68"), Port: 80},
			"",
		},
		{
			"[2001:db8::68]:asdf",
