...
func MakeRequest(ctx context.Context, preq *ParsedHTTPRequest) (*Response, error) {
			...
			if currentContentEncoding == "" {
				...
				state.Logger.Warn(
					"There's a mismatch between the desired `compression` the manually set `Content-Encoding` header "+
						"in the request, the custom header has precedence and won't be overwritten. "+
						"This may result in invalid data being sent to the server.",
					log.String("method", preq.Req.Method), log.Stringer("url", preq.Req.URL))
			}
		}
	...
	if contentLengthHeader := preq.Req.Header.Get("Content-Length"); contentLengthHeader != "" {
		...
		if err != nil || preq.Req.ContentLength != int64(length) {
			state.Logger.Warn(
				"The specified Content-Length header in the request "+
					"doesn't match the actual request body length, so it will be ignored!",
				log.String("content-length-head", contentLengthHeader),
				log.String("method", preq.Req.Method),
				log.Stringer("url", preq.Req.URL),
				log.Int64("content-length", preq.Req.ContentLength))
		}
	}
	...
	combinedLogFields := []log.Field{
		log.String("source", "http-debug"),
	}
	for k, v := range tags {
		combinedLogFields = append(combinedLogFields, log.String(k, v))
	}
	...
	if state.Options.HTTPDebug.String != "" {
		transport = httpDebugTransport{
			...
			logger:            state.Logger.With(combinedLogFields...),
		}
	}
		...
		CheckRedirect: func(req *http.Request, via []*http.Request) error {
			...
			if l := len(via); int64(l) > preq.Redirects.Int64 {
				if !preq.Redirects.Valid {
					...
					state.Logger.Warn(
						"Stopped after redirects and returned the redirection; pass { redirects: n }"+
							" in request params or set global maxRedirects to silence this",
						log.Stringer("url", url), log.Int("redirects", l))
				}
	...
	if resErr != nil {
		...
		case <-ctx.Done():
		default:
			state.Logger.Warn("Request Failed", log.Error(resErr))
		}
	}
